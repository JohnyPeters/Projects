...
def consulta_BD_shellsort(lista_recebida):
    lista_info=lista_recebida
    start = timeit.default_timer()
        ...
        for h in range(gap):
            for i in range(h,len(lista_indices)+gap,gap):
                for j in range(gap,i,gap):
                    if (lista_indices[i-j - gap][0][0] > lista_indices[i-j][0][0] or (lista_indices[i-j - gap][0][0] == lista_indices[i-j][0][0] and lista_indices[i-j - gap][1] > lista_indices[i-j][1])):
...
while(1):
    ...
    if(cmd[0]=="DIM_BD"):
        ...
        #lista_informacoes = consulta_BD_shellsort(lista_informacoes)
